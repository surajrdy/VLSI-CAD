...
def Final(func, nvars, ncubes):
    if len(func) == 0:
        ...
        P_n = Final(P, nvars, ncubes)
        N_n = Final(N, nvars, ncubes)
        P_c = AND(z, P_n, 'true')
        N_c = AND(z, N_n, 'comp')
        answer = OR(P_c, N_c)
        return answer
...
def BinateVariable(function, nvars, ncubes):
    ...
    for x in range(ncubes-1):
        for y in range(nvars-1):
            if cubes[x][y] == '01':
                true_count[y] += 1
            elif cubes[x][y] == '10':
                neg_count[y] += 1
    ...
    for i in range(ncubes):
        if function[i][mostbinate-1] != terminator:
            ...
            for j in range(len(function[i])):
                new_cube[cube_count-1][j] = function[i][j]
                if j == mostbinate-1:
                    if function[i][j] == opposite:
                        new_cube[cube_count-1][j] = '11'
